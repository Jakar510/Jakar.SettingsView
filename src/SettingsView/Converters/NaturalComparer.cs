...
namespace Jakar.SettingsView.Shared.Converters
{
	...
	#region Property
		...
		public virtual NaturalComparerOptions Options
		{
			...
			set
			{
				...
				else { _ignoreTable = _ignoreCharacter; }
			}
		}
	...
	#region Method
		...
		/// </list>
		/// </returns>
		public int Compare( string s1, string s2 ) => LocalCompare(s1, s2) * _order;
		...
		/// </list>
		/// </returns>
		int IComparer.Compare( object s1, object s2 ) => LocalCompare(s1 as string, s2 as string) * _order;
		...
		protected virtual int LocalCompare( string s1, string s2 )
		{
			...
			else if ( string.IsNullOrEmpty(s2) ) { return 1; }
			...
			while ( p1 < s1.Length && p2 < s2.Length )
			{
				...
				// いずれかが数字の場合
				else if ( ( t1 != CharTypes.None || t2 != CharTypes.None ) &&
						  t1 != CharTypes.RomanNumber &&
						  t2 != CharTypes.RomanNumber ) { return ( t1 != CharTypes.None ) ? -1 : 1; }
				// 数字でない場合は文字コードを比較する
				else
				{
					int diff = CompareChar(s1[p1], s2[p2]);
			...
			if ( p1 >= s1.Length ) { return ( p2 >= s2.Length ) ? 0 : -1; }
			else { return 1; }
		}
		...
		private CharTypes GetCharType( char c, char back, CharTypes state )
		{
			// ASCIIアラビア数字 (0～9)
			if ( c >= '0' &&
				 c <= '9' ) { return CharTypes.Number; }
			// 日本語アラビア数字 (０～９)
			else if ( c >= '０' &&
					  c <= '９' ) { return CharTypes.Number; }
			// 日本語丸数字 (①～⑳)
			else if ( c >= '①' &&
					  c <= '⑳' ) { return CharTypes.CircleNumber; }
			// ASCII英大文字 (A～Z)
			else if ( c >= 'A' &&
					  c <= 'Z' )
			{
			...
			// ローマ数字
					  ...
					  c <= 0x217F ) { return CharTypes.JpRomanNumber; }
			else
			{
				// 日本語漢数字
		...
		private void ConvertHalf( StringBuilder source )
		{
			...
			{
				if ( source[i] >= '！' &&
					 source[i] <= '～' ) { source[i] = (char) ( source[i] - '！' + '!' ); }
				else
				{
					switch ( source[i] )
					{
		...
		private void ConvertUpperCase( StringBuilder source )
		{
			for ( var i = 0; i < source.Length; i++ )
			{
				if ( ( source[i] >= 'a' && source[i] <= 'z' ) &&
					 ( source[i] >= 'ａ' && source[i] <= 'ｚ' ) ) { source[i] = char.ToUpperInvariant(source[i]); }
			}
		}
		...
		private void ConvertKatakana( StringBuilder source )
		{
			...
			{
				if ( source[i] >= 'ぁ' &&
					 source[i] <= 'ゞ' ) { source[i] = (char) ( source[i] + 'ァ' - 'ぁ' ); }
				else if ( source[i] >= 'ｦ' &&
						  source[i] <= 'ﾟ' )
				{
					var replaced = false;
					...
					if ( i + 1 < source.Length )
					{
						...
						if ( replaced ) { source.Remove(i + 1, 1); }
					}
		...
		private bool GetNumber( string source,
								CharTypes type,
								ref int pos,
								out long value )
		{
			...
			{
				if ( number.AddChar(source[i]) == false ) { break; }
			}
	...
	#region RomanNumberConverter
		...
		#region Property
			...
			/// <summary>数字から変換した数値を取得します。</summary>
			public long Value => _value + _number;
		...
		#region Method
			...
			protected bool IsAlpha( char alpha ) => ( ( alpha >= 'A' && alpha <= 'Z' ) || ( alpha >= 'a' && alpha <= 'z' ) || ( alpha >= 'Ａ' && alpha <= 'Ｚ' ) || ( alpha >= 'ａ' && alpha <= 'ｚ' ) );
	...
	#region JpRomanNumberConverter
		...
		#region Constructor
			...
			public JpRomanNumberConverter( char roman )
			{
				...
				if ( roman >= 0x2160 &&
					 roman <= 0x216F ) { _romanOne = (char) 0x2160; }
				// 全角ローマ数字(ⅰ～xii,l,c,d,m)
				else if ( roman >= 0x2170 &&
						  roman <= 0x217F ) { _romanOne = (char) 0x2170; }
		...
		#region Method
			...
			public bool AddChar( char roman )
			{
				if ( _isMultiChar == false ) { return false; }
	...
	#region CircleNumberConverter
		...
		#region Constructor
			...
			public CircleNumberConverter( char number )
			{
				// ①～⑳
				if ( number >= 0x2460 &&
					 number <= 0x2473 ) { _number = number - 0x2460 + 1; }
				// (1)～(12)
				else if ( number >= 0x2474 &&
						  number <= 0x2487 ) { _number = number - 0x2474 + 1; }
				// 1.～20.
				else if ( number >= 0x2488 &&
						  number <= 0x249B ) { _number = number - 0x2488 + 1; }
				// 丸付き21～35
				else if ( number >= 0x3251 &&
						  number <= 0x325F ) { _number = number - 0x3251 + 21; }
				// {一}～{十}
				else if ( number >= 0x3220 &&
						  number <= 0x3229 ) { _number = number - 0x3220 + 1; }
				// 丸付き一～十
				else if ( number >= 0x3280 &&
						  number <= 0x3289 ) { _number = number - 0x3280 + 1; }
			}
		...
		#region Method
			...
			/// <param name="number">数字を表す char。</param>
			/// <returns>数字として成立する場合は <see langword="true"/>、それ以外の場合は <see langword="false"/>。</returns>
			public bool AddChar( char number ) => false;
	...
	#region KanjiNumberConverter
		...
		#region Property
			...
			/// <summary>数字から変換した数値を取得します。</summary>
			public long Value => _value2 + _value1 + _number;
